 ...
 */
package apktool.brut.androlib.res.decoder;

import apktool.brut.androlib.AndrolibException;
import apktool.brut.androlib.err.AXmlDecodingException;
import apktool.brut.androlib.err.RawXmlEncounteredException;
  ...
  public void decode(InputStream in, OutputStream out)
    ...
    try {
      ...
      XmlSerializerWrapper ser = new StaticXmlSerializerWrapper(mSerial, factory) {
        ...
        boolean appNameResourceFound = false;
          ...
          if (type == XmlPullParser.START_TAG) {
            if ("manifest".equalsIgnoreCase(pp.getName())) {
              ...
              try {
                ...
                (!appNameResourceFound && "activity".equalsIgnoreCase(pp.getName()))) {
              appNameResourceFound = setAppNameResource(pp);
            }
        ...
        private boolean setAppNameResource(XmlPullParser pp) {
          String attr_name;

          // read element for label attribute
          for (int i = 0; i < pp.getAttributeCount(); i++) {
            attr_name = pp.getAttributeName(i);

            if (attr_name.equalsIgnoreCase(("label"))) {
              manageAppName(pp.getAttributeValue(i));
              return true;
            }
          }
          return false;
        ...
        private void manageAppName(String attributeValue){
          if(attributeValue.startsWith(RESOURCE_NAME_PREFIX)){
            resTable.setAppNameResource(attributeValue.replaceAll(RESOURCE_NAME_PREFIX, ""));
          } else {
            resTable.setAppName(attributeValue);
          }
        }
