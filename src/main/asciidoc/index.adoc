...
Questo servizio registra sul server un file di un'applicazione. In questo caso il servizio accetta in input semplicemente il file dell'applicazione e provvede a estrarne automaticamente le informazioni identificative quali package, nome e versione. Il nome del file salvato sul server sarà quello originale del file inviato nella richiesta. Per poterlo ottenere automaticamente, questo servizio accetta richieste _multipart_ e quindi il formato _multipart/form-data_. Il file deve essere inviato nella richiesta HTTP in una _part_ denominata *_file_*. Se non si verificano errori durante l'elaborazione della richiesta, il servizio risponde con un oggetto JSON contenente tutte le informazioni dell'applicazione appena installata.
Risponde all'indirizzo
...
Questo servizio permette di installare dei frameworks per decodificare files apk con risorse specifiche. Per maggior informazioni sui frameworks si veda la documentazione originale del tool di decodifica usato in questa applicazione
...
===== Framework invalido
Nel caso in cui il framework inviato non sia valido e non sia possibile installarlo, il serivzio risponde con una HTTP response vuota e HTTP status 406.
...
Risponde all'indirizzo
*_api/v1/configuration/installedFrameworks_*
...
Questo servizio permette di attivare, disattivare e modificare la configurazione del limitatore di banda a runtime per la corrente esecuzione di Package Manager. Si noti che la configurazione inviata tramite il servizio *non è permanente*, al riavvio di Package Manager verranno ricaricati i valori di default definiti nel file _application.properties_  (si veda la documentazione di installazione e configurazione nel file README.md).
Il servizio risponde al metodo *HTTP PATCH* all'indirizzo
*_api/v1/configuration/bandwidthLimiter_*
...
Risponde al metodo *HTTP GET* dell'indirizzzo
*_api/v1/configuration/bandwidthLimiter_*
