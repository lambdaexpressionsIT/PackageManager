...
* *bandwidth.limitation.enabled*: booleano che attiva il servizio di limitazione della banda per l'applicazione
* *bandwidth.max.kbitPerSecond*: banda massima allocata per l'applicazione (in kbps).
* *download.max.kbitPerSecond*: velocità massima consentita di download dall'applicazione(in kbps)
* *upload.max.kbitPerSecond*: velocità massima consentita per l'upload verso l'applicazione(in kbps)
